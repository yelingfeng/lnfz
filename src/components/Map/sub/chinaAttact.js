/**
 * Created by yelingfeng on 2016/9/14.
 */
import * as helper from "../helper"
const chinaGeoCoord = {
    '辽宁': [123.1238,42.1216],
    '黑龙江':[127.9688,45.368],
    '吉林':[125.8154,44.2584],
    '河北':[114.4995,38.1006],
    '河南':[113.4668,34.6234],
    '陕西':[109.1162,34.2004],
    '山西':[112.3352,37.9413],
    '山东':[117.1582,36.8701],
    '甘肃':[103.5901,36.3043],
    '云南':[102.9199,25.4663],
    '贵州':[106.6992,26.7682],
    '福建':[119.4543,25.9222],
    '广东':[113.5107,23.2196],
    '海南':[110.3893,19.8516],
    '四川':[103.9526,30.7617],
    '湖北':[114.3896,30.6628],
    '湖南':[113.0823,28.2568],
    '江西':[116.0046,28.6633],
    '安徽':[117.29,32.0581],
    '江苏':[118.8062,31.9208],
    '浙江':[119.5313,29.8773],
    '青海':[101.4038,36.8207],
    '新疆':[87.9236,43.5883],
    '内蒙古':[111.4124,40.4901],
    '宁夏':[106.3586,38.1775],
    '西藏':[91.1865,30.1465],
    '广西':[108.479,23.1152],


    '上海': [121.4648,31.2891],
    '朔州' :[112.43,39.33],
    '阳曲' :[ 112.65 , 38.05 ],
    '晋中' : [112.75,37.68],
    '吕梁' : [111.13,37.52],
    '十堰' : [110.78 ,32.65],
    '辽阳' :[123.2437260000,41.2744520000],
    '锦州' :[121.1336310000,41.1008690000],
    '朝阳' :[120.4573010000,41.5794870000],
    '本溪' :[123.7734680000,41.2998470000],
    '鞍山' :[123.0009740000,41.1141220000],
    '铁岭' :[123.8487970000,42.2925730000],
    '阜新' :[121.6765180000,42.0279830000],
    '抚顺' :[123.9635950000,41.8860780000],
    '娄烦' :[ 111.78 , 38.05 ],
    '清徐' :[ 112.33 , 37.62 ],
    '阳泉' :[ 113.57 , 37.85 ],
    '天镇' :[ 114.08 , 40.42 ],
    '灵丘' :[ 114.2  , 39.47 ],
    '怀仁' :[ 113.1  , 39.82 ],
    '山阴' :[ 112.82 , 39.52 ],
    '平鲁' :[ 112.12 , 39.53 ],
    '右玉' :[ 112.33 , 40.18 ],
    '阳高' :[ 113.72 , 40.38 ],
    '广灵' :[ 113.27 , 39.75 ],
    '浑源' :[ 113.68,  39.7],
    '应县' :[ 113.18 , 39.58],
    '朔县' :[ 112.42 , 39.32 ],
    '左云' :[ 112.67 , 40.02 ],
    '忻县' :[ 112.7 ,  38.38 ],
    '代县' :[ 112.97 , 39.07 ],
    '五台' :[ 113.32 , 38.72 ],
    '静乐' :[ 111.9  , 38.37 ],
    '保德' :[ 111.09 , 38.01 ],
    '河曲' :[ 111.17 , 39.38 ],
    '神池' :[ 112.17 , 39.1],
    '原平' :[ 112.7  , 38.73],
    '繁峙' :[ 113.28 , 39.2],
    '定襄' :[ 112.95 , 38.5],
    '岢岚' :[ 111.58 , 38.7],
    '五寨' :[ 111.82 ,38.93] ,
    '偏关' :[ 111.47 , 39.45 ],
    '宁武' :[ 112.28 , 39],
    '榆次' :[ 112.72 , 37.68],
    '孟县' :[ 113.37 , 38.01],
    '昔阳' :[ 113.68 , 37.62],
    '左权' :[ 113.35 , 37.07],
    '太谷' :[ 112.53 , 37.42 ],
    '平遥' :[ 112.18 , 37.2],
    '灵石' :[ 111.77 , 36.83],
    '寿阳' :[ 113.17 , 37.88 ],
    '平定' :[ 113.62 , 37.79 ],
    '和顺' :[ 113.55 , 37.33 ],
    '榆社' :[ 112.97 , 37.08 ],
    '祁县' :[ 112.33 , 37.36 ],
    '介休' :[ 111.88 , 37.03 ],
    '离石' :[ 111.13 , 37.53 ],
    '兴县' :[ 111.22 , 38.47 ],
    '方由' :[ 111.24 , 37.86 ],
    '岚县' :[ 111.62 , 38.28 ],
    '交城' :[ 112.14 , 37.55 ],
    '文水' :[ 112.02 , 37.42 ],
    '汾阳' :[ 111.75 , 37.27 ],
    '孝义' :[ 111.8  , 37.12 ],
    '交口' :[ 111.2  , 36.97 ],
    '石楼' :[ 110.83 , 37],
    '中阳' :[ 111.17 , 37.37],
    '临县' :[ 110.95 , 37.95 ],
    '柳林' :[ 110.85 , 37.45 ],
    '襄垣' :[ 113.02 , 36.55 ],
    '黎城' :[ 113.4  , 36.56 ],
    '壶关' :[ 113.23 , 35.11 ],
    '高平' :[ 112.88 , 35.48 ],
    '阳城' :[ 112.38 , 35.84 ],
    '长子' :[ 112.87 , 36.13 ],
    '沁源' :[ 112.32 , 36.5],
    '潞城' :[ 113.22 , 36.33],
    '武乡' :[ 112.83 , 36.83 ],
    '平顺' :[ 113.43 , 36.19 ],
    '陵川' :[ 113.27 , 35.78 ],
    '晋城' :[ 112.83 , 35.52 ],
    '沁水' :[ 112.15 , 35.67 ],
    '屯留' :[ 112.87 , 36.32 ],
    '沁县' :[ 112.68 , 36.75 ],
    '临汾' :[ 111.5  , 36.08 ],
    '汾西' :[ 111.53 , 36.63 ],
    '安泽' :[ 112.2  , 36.15 ],
    '古县' :[ 111.9  , 36.29 ],
    '翼城' :[ 111.68 , 35.73 ],
    '曲沃' :[ 111.33 , 35.63 ],
    '吉县' :[ 110.65 , 36.12 ],
    '大宁' :[ 110.72 , 36.47 ],
    '侯马' :[ 111.45 , 35.03 ],
    '永和' :[ 110.64 , 36.62 ],
    '洪洞' :[ 111.68 , 36.25 ],
    '霍县' :[ 111.72 , 36.57 ],
    '浮山' :[ 111.83 , 35.97 ],
    '襄汾' :[ 111.43 , 35.86 ],
    '乡宁' :[ 110.8  , 35.97 ],
    '蒲县' :[ 111.07 , 36.42 ],
    '运城' :[ 110.97 , 35.03 ],
    '闻喜' :[ 111.2  , 35.37 ],
    '垣曲' :[ 111.63 , 35.3],
    '芮城' :[ 110.68 , 34.71] ,
    '临猗' :[ 110.78 , 35.15 ],
    '新绛' :[ 111.22 , 35.62 ],
    '河津' :[ 110.7  , 35.58 ],
    '夏县' :[ 111.22 , 35.12 ],
    '绛县' :[ 111.58 , 35.48 ],
    '平陆' :[ 111.2  , 34.12 ],
    '永济' :[ 110.42 , 34.88 ],
    '万荣' :[ 110.83 , 35.42 ],
    '稷山' :[ 110.97 , 35.6],
    '东莞': [113.8953,22.901],
    '东营': [118.7073,37.5513],
    '中山': [113.4229,22.478],
    '临汾': [111.4783,36.1615],
    '晋城': [112.8480,35.5018],
    '临沂': [118.3118,35.2936],
    '丹东': [124.541,40.4242],
    '忻州': [112.43,38.24],
    '丽水': [119.5642,28.1854],
    '台湾' : [121.5365, 25.0192],
    '香港' :[114.1000, 22.2000],
    '乌鲁木齐': [87.9236,43.5883],
    '佛山': [112.8955,23.1097],
    '保定': [115.0488,39.0948],
    '兰州': [103.5901,36.3043],
    '包头': [110.3467,41.4899],
    '北京': [116.4551,40.2539],
    '北海': [109.314,21.6211],
    '南京': [118.8062,31.9208],
    '南宁': [108.479,23.1152],
    '南昌': [116.0046,28.6633],
    '南通': [121.1023,32.1625],
    '厦门': [118.1689,24.6478],
    '台州': [121.1353,28.6688],
    '合肥': [117.29,32.0581],
    '呼和浩特': [111.4124,40.4901],
    '咸阳': [108.4131,34.8706],
    '哈尔滨': [127.9688,45.368],
    '唐山': [118.4766,39.6826],
    '嘉兴': [120.9155,30.6354],
    '大同': [113.7854,39.8035],
    '大连': [122.2229,39.4409],
    '天津': [117.4219,39.4189],
    '太原': [112.3352,37.9413],
    '威海': [121.9482,37.1393],
    '宁波': [121.5967,29.6466],
    '宝鸡': [107.1826,34.3433],
    '宿迁': [118.5535,33.7775],
    '常州': [119.4543,31.5582],
    '广州': [113.5107,23.2196],
    '廊坊': [116.521,39.0509],
    '延安': [109.1052,36.4252],
    '张家口': [115.1477,40.8527],
    '徐州': [117.5208,34.3268],
    '德州': [116.6858,37.2107],
    '惠州': [114.6204,23.1647],
    '成都': [103.9526,30.7617],
    '扬州': [119.4653,32.8162],
    '承德': [117.5757,41.4075],
    '拉萨': [91.1865,30.1465],
    '无锡': [120.3442,31.5527],
    '日照': [119.2786,35.5023],
    '昆明': [102.9199,25.4663],
    '杭州': [119.5313,29.8773],
    '枣庄': [117.323,34.8926],
    '柳州': [109.3799,24.9774],
    '株洲': [113.5327,27.0319],
    '武汉': [114.3896,30.6628],
    '汕头': [117.1692,23.3405],
    '江门': [112.6318,22.1484],
    '沈阳': [123.1238,42.1216],
    '沧州': [116.8286,38.2104],
    '河源': [114.917,23.9722],
    '泉州': [118.3228,25.1147],
    '泰安': [117.0264,36.0516],
    '泰州': [120.0586,32.5525],
    '济南': [117.1582,36.8701],
    '济宁': [116.8286,35.3375],
    '海口': [110.3893,19.8516],
    '淄博': [118.0371,36.6064],
    '淮安': [118.927,33.4039],
    '深圳': [114.5435,22.5439],
    '清远': [112.9175,24.3292],
    '温州': [120.498,27.8119],
    '渭南': [109.7864,35.0299],
    '湖州': [119.8608,30.7782],
    '湘潭': [112.5439,27.7075],
    '滨州': [117.8174,37.4963],
    '潍坊': [119.0918,36.524],
    '烟台': [120.7397,37.5128],
    '玉溪': [101.9312,23.8898],
    '珠海': [113.7305,22.1155],
    '盐城': [120.2234,33.5577],
    '盘锦': [121.9482,41.0449],
    '石家庄': [114.4995,38.1006],
    '福州': [119.4543,25.9222],
    '秦皇岛': [119.2126,40.0232],
    '绍兴': [120.564,29.7565],
    '聊城': [115.9167,36.4032],
    '肇庆': [112.1265,23.5822],
    '舟山': [122.2559,30.2234],
    '苏州': [120.6519,31.3989],
    '莱芜': [117.6526,36.2714],
    '菏泽': [115.6201,35.2057],
    '营口': [122.4316,40.4297],
    '葫芦岛': [120.1575,40.578],
    '衡水': [115.8838,37.7161],
    '衢州': [118.6853,28.8666],
    '西宁': [101.4038,36.8207],
    '西安': [109.1162,34.2004],
    '贵阳': [106.6992,26.7682],
    '连云港': [119.1248,34.552],
    '邢台': [114.8071,37.2821],
    '邯郸': [114.4775,36.535],
    '郑州': [113.4668,34.6234],
    '鄂尔多斯': [108.9734,39.2487],
    '重庆': [107.7539,30.1904],
    '金华': [120.0037,29.1028],
    '铜川': [109.0393,35.1947],
    '银川': [106.3586,38.1775],
    '镇江': [119.4763,31.9702],
    '长春': [125.8154,44.2584],
    '长沙': [113.0823,28.2568],
    '长治': [112.8625,36.4746],
    '阳泉': [113.4778,38.0951],
    '青岛': [120.4651,36.3373],
    '韶关': [113.7964,24.7028],
    '桂林': [110.290195,25.273566],
    '黄冈': [115.5111,30.4022],
    '自贡': [104.7011,29.1342]
}
/**
 * 获取空圈大小值的方法
 * @param v 当前值
 * @param maxmin 整体值
 */
function getMakerEmptyCircleV(v,maxmin){
    var config = {
        max : 50,
        min : 10
    }
    var min = maxmin.min;
    var max = maxmin.max;
    if(v > config.max){
        v = Math.round( parseFloat(v /max  )* config.max);
    }

    if( v < config.min){
        v = 10 + v*2;
    }
    v = parseInt(v,10);

    return v;
}

function getSymbolSize(type){
    let n = type == undefined ? "5" : type*0.5 + 1;
    return [n,n]
}

function getAttactData(data){
    var lineData = [];
    var pointData = [];
    data.forEach(function(row){
        var __data = [];
        if(chinaGeoCoord[row.name] && chinaGeoCoord[row.targetName] ){
            lineData.push({
                fromName: row.name,
                toName:row.targetName,
                value : row.value,
                coords: [chinaGeoCoord[row.name], chinaGeoCoord[row.targetName]]
            });
            pointData.push({name:row.targetName,value:chinaGeoCoord[row.targetName],symbolSize:getSymbolSize(row.lineType)})
        }

    });
    return {
        lineData,
        pointData
    }
}

function  getAttactMapSeries(data){

    //#ed6d42'
    var color = '#a6c84c';
    var aData = getAttactData(data);
    let seriesArr = []
    seriesArr.push({
        type: 'lines',
        data :aData.lineData,
        lineStyle:{
            normal: {
                width:1,
                color : "rgba(255,255,255,0.05)",
                opacity: 0.3,
                curveness: 0.4
            }
        },
        smooth:true,
        effect: {
            show: true,
            scaleSize: 1.5,
            period:5,
            color: color,
            symbol: 'arrow',
            symbolSize : 3,
            shadowBlur: 5
        }
    });

    seriesArr.push({
        type: 'effectScatter',
        coordinateSystem: 'geo',
        rippleEffect: {
            brushType: 'stroke',
            scale:3
        },
        label: {
            normal: {
                show: true,
                position: ['8', '8'],
                formatter: '{b}',
            }
        },
        symbolSize: function (val) {
            return  [10,10];
        },
        itemStyle: {
            normal: {
                color: color
            }
        },
        data:aData.pointData
    });


    return seriesArr;
}

function getMapRangeOption(){
    var dataRange = {
        color  :['rgba(25,143,212,0.05)','rgba(41,164,101,0.05)','rgba(231,141,0,0.05)','rgba(228,74,0,0.05)','rgba(51,185,218,0.05)','rgba(173,51,218,0.05)'],
        show : true,
        splitList: [
            {start: 10 ,color:"#198fd4"},
            {start: 8, end: 10,color:"#29a465"},
            {start: 6, end: 8, color:"#e78d00"},
            {start: 4, end: 6, color:"#e44a00"},
            {start: 2, end: 3, color:"#33b9da"},
            {end: 1, color:"#ad33da"}
        ],
        textStyle :{
            color : "#fff"
        }
    }
    return dataRange;
}


function setting(option, props) {

    let op = {

        geo: {
            map: 'china',
            roam: true,
            silent: true,
            itemStyle: {
                normal: {
                    borderWidth: 0.5,
                    areaColor: 'transparent',
                    borderColor: "rgba(126, 220, 248,0.95)",
                    //shadowColor: 'rgba(255,255,255,0.9)',
                    //shadowBlur: 2
                }
            },
        },
        tooltip : {
            trigger: 'item',
            backgroundColor:"transparent",
            formatter : function(params){
                var data = params.data;
                var name = params.name ;
                if(name !== ""){
                    return name ;
                }
                if(data!=null){
                    var str = data.fromName +" -> "+ data.toName +"<br>" + data.value +"个";
                    return helper.getMapTipBox(str);
                }
                return '';
            },
            textStyle : {
                fontSize : 12
            }
        },
        series: getAttactMapSeries(option),
    }

    return op;
}
export default  {
    setting
}